classdef PlateLoader < hgsetget
    ...
    methods
        ...
        function response = getStatus(obj)
            ...
            response = getResponse(obj);
            splitString = strsplit(response,' ');
            if(~strcmp('READY',splitString(1)))
                return;
            end
            for i = 1:1:max(size(splitString))
                if(strcmp('POSITION',splitString(i)))
                    temp = splitString(i+1);
                    obj.xAxisPosition = str2num(temp);
                elseif(strcmp('Z-AXIS', splitString(i)))
                    temp = splitString(i+1);
                    if(strcmp('EXTENDED', temp))
                        obj.isZAxisExtended = true;
                    else
                        obj.isZAxisExtended = false;
                    end
                elseif(strcmp('GRIPPER', splitString(i)))
                    temp = splitString(i+1);
                    if(strcmp('CLOSED', temp))
                        obj.isGripperClosed = true;
                    else
                        obj.isGripperClosed = false;
                    end
                elseif(strcmp('PLATE', splitString(i)))
                    obj.isPlatePresent = true;
                elseif(strcmp('NOPLATE', splitString(i)))
                    obj.isPlatePresent = false;
                end
            end
        end
        ...
        function response = getVersion(obj)
            fprintf(obj.serialRobot, 'VERSION');
            response = getResponse(obj);
        end
